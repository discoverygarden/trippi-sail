...
public class SesameTripleIterator extends TripleIterator {
    ...
    public SesameTripleIterator(QueryLanguage lang, String queryText, RepositoryConnection connection, Dataset dataset)
            throws TrippiException, RepositoryException, MalformedQueryException {
    ...
    public Triple next() throws TrippiException {
        try {
            Statement next;
            next = result.next();
            return triple(next.getSubject(), next.getPredicate(), next.getObject());
        }
        catch (Exception e) {
    ...
    private Triple triple(org.openrdf.model.Resource subject, org.openrdf.model.URI predicate,
            org.openrdf.model.Value object) throws IOException {
        try {
            return m_util.createTriple(subjectNode(subject), predicateNode(predicate), objectNode(object));
        }
        catch (Exception e) {
